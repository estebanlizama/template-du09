...
EXECUTE secgen_db.Analisis2.sg_fupssSecgen16
    @rut_person = '070671514',
    @cod_contra = 3907

-- Validar previamente los contratos disponibles del funcionario:
SELECT
    pers.rut_person,
    cont.cod_contra,
    cont.cod_unidad,
...
FROM sisper_db.dbo.sp_pers pers,
     sisper_db.dbo.sp_cont cont,
     ufro_db.dbo.es_unid unid
WHERE pers.cod_ficha = cont.cod_ficha
  AND cont.cod_unidad = unid.cod_unidad
  AND pers.rut_person = '070671514'
ORDER BY cont.principal DESC, cont.cod_contra

-- Diagnostico del contrato y su cadena de jefatura:
SELECT
    pers.rut_person,
    cont.cod_ficha,
    cont.cod_contra,
...
WHERE pers.rut_person = '070671514'
  AND cont.cod_contra = 3907
